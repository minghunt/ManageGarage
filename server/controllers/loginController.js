        ...
        if (existingToken) {
          try {
            ...
            if (decoded && decoded.exp > Math.floor(Date.now() / 1000)) {
              ...
              res.status(200).json({ message: "Đăng nhập thành công", token: extendedToken ,userRole });
            } else {
              ...
              res.status(200).json({ message: "Đăng nhập thành công", token: newToken ,userRole });
            }
          } catch (error) {
          ...
          // Set the new token in the cookie
          res.status(200).json({ message: "Đăng nhập thành công", token ,userRole });
        }
      } else {
